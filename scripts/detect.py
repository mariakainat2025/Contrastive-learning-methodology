"""
scripts/detect.py  —  Stage 9: CLIP+FILIP Anomaly Detection (Score Only)

Test windows:
  Malicious  : window_3     (THEIA original — phase 3, always available)
               file1_window (THEIA file1.json — phishing → credential theft, needs process_file1)
  Benign     : benign_8 (THEIA original)

Two-stage detection strategy (CLIProv + FILIP):
  Stage 1 — CLIP coarse retrieval (global similarity):
    clip_score = max_{j} sim(z_G, z_T_j) - sim(z_G, z_benign)

  Stage 2 — FILIP fine-grained re-ranking (Yao et al., arXiv:2111.07783):
    filip_score = max_k(filip_sim(G, T_k)) - filip_benign_sim(G)
...
Paper attributions:
  ...
  FILIP (Yao 2021)    — token-wise maximum similarity for fine-grained re-ranking
"""
import os
import sys
import json
...
from scripts.encode_features import build_node_text as _build_node_text


TEST_BENIGN_KEYS  = ['benign_8']
TEST_MAL_ORIG     = ['file1_window']   # window_3 always present; file1_window needs process_file1 first
...
# Training split keys (for FILIP score computation only)
...
def filip_sim_precomputed(z_G_nodes, z_T_tokens):
    """FILIP score: mean over nodes of each node's max token similarity."""
...
def filip_explain(z_G_nodes, z_T_tokens, node_texts, cti_tokens, top_n=5):
    ...
    for i in range(sim_matrix.shape[0]):
        ...
        # pick best token that is not pure punctuation
...
def run_contrastive_test():
    """Evaluate trained projectors — show raw scores only, no threshold/metrics."""
    ...
    # In real-time we receive a window without knowing its label.  All routing
    # is done by inspecting the stored data format:
    #   list  → seed-subgraph format  (any newly processed window)
    #   dict  → single-block format   (pre-stored benign baseline windows)
    # truth_str is kept only for display / offline evaluation and plays no part
    # in how embeddings are extracted or scores are computed.
    all_graph_emb = {}
    all_graph_emb.update(benign_emb_all)   # benign baselines
    all_graph_emb.update(window_emb_all)   # new / suspicious windows (overwrite if clash)
    ...
    # Every window is looked up from the same merged dicts regardless of label.
    # truth_str is only used for terminal display during offline evaluation.
    # In real-time inference, pass truth_str='unknown'.
    test_cases = []
    ...
    with torch.no_grad():
        ...
        for wkey, truth_str, emb_dict, node_dict, dataset_tag in test_cases:
            ...
            if can_filip:
                ...
                # Route purely on stored data format — no truth label needed.
                #   list  → seed-subgraph format: one dict per IoC seed subgraph.
                #           Produced by embed_windows.py and process_file1.py.
                #   dict  → single-block format: one flat node_emb tensor.
                #           Produced by embed_windows.py for benign windows.
                if isinstance(entries, list):
                    ...
                    can_filip = False   # no node embeddings available for this window
            ...
            if isinstance(node_dict.get(wkey), list):   # seed-subgraph format
