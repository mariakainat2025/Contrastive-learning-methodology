...
class GraphProjector(nn.Module):
    ...
    def __init__(self, in_dim=64, hidden_dim=256, out_dim=128):
        super().__init__()
...
TRAIN_MAL_KEYS    = ['window_1',, 'window_2', 'window_3']
TRAIN_CTI_KEYS    = ['theia1',   'theia2',   'theia3']
TRAIN_BENIGN_KEYS = ['benign_1', 'benign_2', 'benign_3', 'benign_4', 'benign_5', 'benign_6']
...
def run_contrastive_train():
    ...
    Paper attributions:
      ...
      TRAP               — Adam lr=0.001, weight_decay=5e-4
    """
    ...
    # one shared benign text for all benign windows (CLIProv strategy)
    ...
    # logit_scale IS a learnable parameter — CLIP trains it so the model can
    # adjust the sharpness of the similarity distribution automatically.
    # Clamped to [0, log(100)] to prevent numerical overflow (CLIP convention).
    logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
    logit_scale  = logit_scale.to(device)

    #  optimizer — projectors + logit_scale (all three are learnable)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for pair_idx in range(n_mal):
            ...
            for b_idx in range(n_benign):
                ...
                # save first batch to reuse for final  display
        ...
        # The benign-malicious loop above never teaches the model to distinguish
        # window_1→theia1 vs window_1→theia2. This N=3 batch trains the full
        # attack-phase matching matrix simultaneously: it forces each window
        # to prefer its own CTI report over the other attack CTI reports.
        # This is the core CLIP-style multi-class alignment objective.
        if n_mal >= 2:
            z_G_all = F.normalize(g_proj(mal_graph_train), dim=-1)
            z_T_all = F.normalize(t_proj(mal_text_train),  dim=-1)
            loss_mal = info_nce_loss(z_G_all, z_T_all, logit_scale)
            optimizer.zero_grad()
            loss_mal.backward()
            optimizer.step()
            with torch.no_grad():
                logit_scale.clamp_(0, np.log(100))
            epoch_losses.append(loss_mal.item())
    ...
    #    show final model step-by-step on same first batch                     ─
    ...
    with torch.no_grad():
        train_cti_vecs = torch.stack(
            [cti_emb_all[k] for k in TRAIN_CTI_KEYS]).to(device)
        z_cti_train = F.normalize(t_proj(train_cti_vecs), dim=-1)
        ...
        train_scores_mal = []
        for k in set(TRAIN_MAL_KEYS):
            g_vec = window_emb_all[k].unsqueeze(0).to(device)
            z_G   = F.normalize(g_proj(g_vec), dim=-1).squeeze(0)
            # Adapted from: https://github.com/openai/CLIP  (clip/clip.py)
            # OpenAI CLIP zero-shot scoring: similarity = image_features @ text_features.T
            # Take max over CTI candidates; subtract benign baseline (CLIProv extension).
            score = (z_G @ z_cti_train.T).max().item() - (z_G @ z_benign_text).item()
            train_scores_mal.append(score)

        train_scores_ben = []
        for k in TRAIN_BENIGN_KEYS:
            g_vec = benign_emb_all[k].unsqueeze(0).to(device)
            z_G   = F.normalize(g_proj(g_vec), dim=-1).squeeze(0)
            score = (z_G @ z_cti_train.T).max().item() - (z_G @ z_benign_text).item()
            train_scores_ben.append(score)


    min_mal   = min(train_scores_mal)
    max_ben   = max(train_scores_ben)
    threshold = (min_mal + max_ben) / 2.0

    print(f'\n  Training scores (malicious) : {[round(s,4) for s in train_scores_mal]}')
    print(f'  Training scores (benign)    : {[round(s,4) for s in train_scores_ben]}')
    print(f'  Learned threshold           : {threshold:.4f}  '
          f'(midpoint of {min_mal:.4f} and {max_ben:.4f})')
