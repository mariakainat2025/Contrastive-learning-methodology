...
def load_graph(graph_json):
    try:
        ...
        return json_graph.node_link_graph(graph_json, link='links')
...
def semantic_features(G):
    ...
    for n in node_ids:
        ...
        token_lists.append([tag] + hierarchy)
    matrix = hasher.transform(token_lists).toarray()
...
def _process_files(files, graphs_dir, out_dir):
    ...
    for fp in tqdm(files, desc='Building features'):
        ...
        n_ok += 1
    return (n_ok, n_failed, n_skipped, total_featurized_nodes)
...
def batch_main(graphs_dir=GRAPHS_DIR, out_dir=FEATURES_DIR):
    ...
    (n_ok, n_failed, n_skipped, total_featurized_nodes) = _process_files(files, graphs_dir, out_dir)
    print()
    print('Graphs processed  : {} ok, {} failed, {} skipped (already cached)'.format(n_ok, n_failed, n_skipped))
...
def batch_main_scoped(graph_paths, graphs_dir=GRAPHS_DIR, out_dir=FEATURES_DIR):
    files = sorted(graph_paths)
    print('Processing {} scoped graph files'.format(len(files)))
    (n_ok, n_failed, n_skipped, total_featurized_nodes) = _process_files(files, graphs_dir, out_dir)
    print('Graphs processed  : {} ok, {} failed, {} skipped (already cached)'.format(n_ok, n_failed, n_skipped))
    print('Featurized nodes  : {:,} total'.format(total_featurized_nodes))
    print('Saved to         : {}/<tactic>/<technique>/features_<name>.json'.format(out_dir))
