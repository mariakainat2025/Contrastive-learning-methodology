...
from Train_TTP_Recognition_SinglelabelMatched import main as train_main
from Test_TTP_Recognition_SinglelabelMatched import main as test_main
...
def result_path(seed):
    return os.path.join(RESULTS_DIR, 'test_results_singlelabel_matched_seed{}.json'.format(seed))

def summary_path():
    return os.path.join(RESULTS_DIR, 'summary_singlelabel_matched.json')
...
def main(seeds=SEEDS):
    ...
    for seed in seeds:
        print()
        print('=' * 70)
        print('[singlelabel-matched] SEED {}  (same test instances as multilabel seed {})'.format(seed, seed))
        print('=' * 70)
        train_main(seed)
    ...
    print()
    print('=' * 70)
    print('[singlelabel-matched] Summary across {} seeds: {}'.format(len(seeds), list(seeds)))
    print('=' * 70)
