...
def cache_graph_tensors(graph_paths, graphs_dir, out_dir):
    import json
    import os
    n_ok = 0
    n_failed = 0
    ...
    for fp in sorted(graph_paths):
        ...
        out_path = os.path.join(out_dir, os.path.dirname(rel), 'tensors_' + os.path.basename(rel)[:-len('.json')] + '.pt')
        ...
        torch.save({'h': h, 'adjacency': adjacency, 'node_ids': node_ids}, out_path)
        n_ok += 1
    print('Deep tensors cached: {} ok, {} failed, {} skipped (already cached)'.format(n_ok, n_failed, n_skipped))
    print('Saved to           : {}/<tactic>/<technique>/tensors_<name>.pt'.format(out_dir))
