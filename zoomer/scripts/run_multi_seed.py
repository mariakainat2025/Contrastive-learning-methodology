...
from Train_TTP_Recognition import main as train_main
from Test_TTP_Recognition import main as test_main
...
def result_path(seed):
    return os.path.join(RESULTS_DIR, 'test_results_seed{}.json'.format(seed))
...
def main(seeds=SEEDS):
    ...
    for seed in seeds:
        print()
        print('=' * 70)
        print('SEED {}  (independent random split + refit bins/masks + retrain)'.format(seed))
        print('=' * 70)
        train_main(seed)
