...
# ---------------------------------------------------------------------------
# Tactic-graph sequences  (extract_tactics.py output format)
# ---------------------------------------------------------------------------

TACTIC_DATA_ROOT = os.path.join(PROJECT_ROOT, 'output', 'theia', 'tactic_data')
TACTIC_SEQ_OUT   = os.path.join(PROJECT_ROOT, 'output', 'theia', 'tactic_data', 'sequnces')
...
_SKIP_PREFIXES = ('stage3_',)


def _tactic_json_to_sequence(path):
    ...
    lines = []
    for e in edges:
        lines.append('{} {} {}.'.format(e['src_name'], e['etype'], e['dst_name']))
...
def run_tactic_sequences():
    t0 = time.time()
    os.makedirs(TACTIC_SEQ_OUT, exist_ok=True)
    all_sequences = []
    ...
    for folder, attack_label in ATTACK_FOLDERS:
        ...
        files = sorted(
            ...
            if f.endswith('.json') and not any(f.startswith(p) for p in _SKIP_PREFIXES)
        )
        ...
        for fname in files:
            ...
            try:
                entry = _tactic_json_to_sequence(fpath)
                entry['attack_label']  = attack_label
                entry['source_file']   = fname
                all_sequences.append(entry)

                # individual file per tactic
                out_fname = fname.replace('.json', '_sequence.json')
                out_path  = os.path.join(TACTIC_SEQ_OUT, out_fname)
                with open(out_path, 'w', encoding='utf-8') as f:
    ...
    # combined file with all tactics
    combined_path = os.path.join(TACTIC_SEQ_OUT, 'sequences_tactics_all.json')
    with open(combined_path, 'w', encoding='utf-8') as f:
        json.dump(all_sequences, f, indent=2)

    elapsed = time.time() - t0
    print('\nSaved {} tactic sequences → {}'.format(len(all_sequences), TACTIC_SEQ_OUT))
    print('Combined file: sequences_tactics_all.json')
    print('Elapsed: {:.1f}s'.format(elapsed))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode',   default='subgraph',
                        choices=['subgraph', 'tactic'],
                        help='subgraph: original mode; tactic: process extract_tactics.py output')
    parser.add_argument('--input',  help='path to subgraph JSON file (subgraph mode)')
    parser.add_argument('--output', help='path to output sequences JSON (subgraph mode)')
    parser.add_argument('--label',  default='unknown')
    parser.add_argument('--max',    type=int, default=None, help='max subgraphs to process')
    args = parser.parse_args()

    if args.mode == 'tactic':
        run_tactic_sequences()
    else:
        if not args.input or not args.output:
            parser.error('--input and --output are required in subgraph mode')
