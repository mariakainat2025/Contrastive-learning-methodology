...
def build_name_index():
    ...
    for technique_dir in sorted(TECHNIQUES_DIR.iterdir()):
        ...
        for step_dir in sorted(technique_dir.iterdir()):
            ...
            if not has_audit:
                continue
            name_index[step].append(technique)
    return name_index
...
def main():
    ...
    report = []
    for step, techniques in cross_technique.items():
    ...
    print(f"Cross-technique duplicate step names (showing first 20 of {len(report)}):")
