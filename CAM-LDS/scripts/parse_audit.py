...
# Edge types occurring fewer than 10 times across the whole 393-graph corpus
# (confirmed by scanning graphs/*/*/*.json) -- dropping these costs 0 graphs
# under the existing <3-edge training filter (verified: no graph newly drops
# below 3 edges from this removal) while cutting the structural-feature
# dimensionality from 118 (59 types x in/out) to 94 (47 types x in/out).
# EVENT_ANOM_PROMISCUOUS and EVENT_SETXATTR are deliberately NOT included --
# removing them was found to drop graph_3_ssh_apt-14_reposerver.json (a
# vim.basic / DEBIAN-control tampering pattern) below the 3-edge threshold.
RARE_EDGE_TYPES = {
    "EVENT_USER_ERR", "EVENT_FSETXATTR", "EVENT_NETFILTER_nft_unregister_chain",
    "EVENT_NETFILTER_nft_unregister_table", "EVENT_LINK", "EVENT_ADD_USER",
    "EVENT_RENAMEAT", "EVENT_ANOM_ABEND", "EVENT_SYSTEM_SHUTDOWN",
    "EVENT_SYMLINKAT", "EVENT_ADD_GROUP", "EVENT_FCHMOD_FAIL",
}
...
def to_subgraph(events, tactic, attack, description=""):
    ...
    for e in events:
        ...
        if sc == "execve":
            if e["success"] == "yes":
                ...
                dst = proc_node(e["pid"], e["exe"] or e["comm"], ts, cmd=e["command"])
    ...
    edges = [e for e in edges if e[3]["edge_type"] not in RARE_EDGE_TYPES]
