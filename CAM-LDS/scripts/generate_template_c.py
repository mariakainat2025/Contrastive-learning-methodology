...
OUT_DIR     = os.path.join(CAM_LDS_DIR, "templates_dc_c")
...
def folder_to_base_mitre_id(folder_name):
    base, _sub = folder_name.split("-", 1)
    return base


def dataset_base_techniques_for_tactic(tactic):
    folders = sorted(
        os.path.basename(p) for p in glob.glob(os.path.join(GRAPHS_DIR, tactic, "*")) if os.path.isdir(p)
    )
    seen = []
    for f in folders:
        base = folder_to_base_mitre_id(f)
        if base not in seen:
            seen.append(base)
    return seen
...
def main():
    ...
    for tactic in TACTICS:
        base_ids = dataset_base_techniques_for_tactic(tactic)
        sentences = []
        not_found = []
        no_linux = []
        ...
        text = " ".join(sentences)
        out_path = os.path.join(OUT_DIR, f"{tactic}_template_c.txt")
        with open(out_path, "w") as f:
            f.write(text + "\n")

        print(f"{tactic:22s} base_techs={len(base_ids):2d}  "
