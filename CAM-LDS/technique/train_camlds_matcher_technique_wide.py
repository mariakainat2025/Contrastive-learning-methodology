"""
Same as train_camlds_matcher_technique.py, but each sequence's embedding is the text
embedding CONCATENATED with ZOOMER's existing Wide feature vector for that same instance
(node-abstract + edge-type + IoC counts, discretized + cross-product -- see
CAM-LDS/scripts/wide_feature_bridge.py, which reuses CAM-LDS/zoomer/scripts/ directly,
nothing reimplemented). Templates get no wide vector (they're not backed by a graph); text_proj's
output dim is just widened to match the combined sequence embedding size, and the model
learns where to place each template in that larger space during training.
"""
import os
import sys
import json
...
TEMPLATE_DIR      = os.path.join(CAM_LDS_DIR, 'technique', 'templates')
STEP_TACTICS_PATH = os.path.join(CAM_LDS_DIR, 'technique', 'step_tactics.json')
OUTPUT_TRAINING = os.path.join(CAM_LDS_DIR, 'technique', 'checkpoints')
...
ZOOMER_TECHNIQUES = sorted({TECHNIQUE_MERGE.get(t, t) for t in [
    'T1003-008', 'T1018-000', 'T1033-000', 'T1036-005', 'T1053-003', 'T1056-001',
    'T1059-000', 'T1059-004', 'T1070-004', 'T1078-003', 'T1083-000', 'T1105-000',
    'T1219-000', 'T1543-002', 'T1546-000', 'T1548-003',
]})

TEMPLATE_BASE_ID = {t: t.split('-')[0] for t in ZOOMER_TECHNIQUES}
...
def load_step_techniques():
    ...
    for row in report:
        ...
        for occ in row['occurrences']:
            t = TECHNIQUE_MERGE.get(occ['technique'], occ['technique'])
            if t in ZOOMER_TECHNIQUES:
                techniques.add(t)
        step_techniques[row['step']] = sorted(techniques)
...
def load_sequences(min_events=None, return_total=False, sequences_dir=None):
    ...
    entries = [e for e in entries if e['techniques']]
    n_scoped = len(entries)
    if min_events is not None:
        ...
        print('  min_events={}: dropped {}/{} sequences with too few events: {}'.format(
            min_events, len(dropped), n_scoped, dropped))

    multi = [e for e in entries if len(e['techniques']) > 1]
    print('  Loaded {} sequences with >=1 of our {} scoped techniques ({} multi-technique — '
          'kept as multi-label, not dropped). {} total sequences, {} outside scope.'.format(
              len(entries), len(ZOOMER_TECHNIQUES), len(multi), n_total, n_total - n_scoped))
...
def stratified_split(entries, test_size=0.2, seed=SEED, min_test_per_technique=1):
    ...
    test_idx_set = set()
    technique_test_count = {t: 0 for t in all_techniques}

    for technique in all_techniques:
        target = min(min_test_per_technique, max(0, technique_total[technique] - 1))
        for i, e in enumerate(items):
            if technique_test_count[technique] >= target:
                break
            if i in test_idx_set or technique not in e['techniques']:
                continue
            test_idx_set.add(i)
            for t in e['techniques']:
                technique_test_count[t] += 1
...
def load_templates(template_dir=None):
    ...
    for fname in sorted(os.listdir(template_dir)):
        ...
        files_by_base[base] = fname

    templates = {}
    for technique in ZOOMER_TECHNIQUES:
        base = TEMPLATE_BASE_ID[technique]
        fname = files_by_base.get(base)
        if fname is None:
            continue
        with open(os.path.join(template_dir, fname), encoding='utf-8') as f:
            templates[technique] = f.read().strip()
    return templates
...
def encode_sequence_combined(tokenizer, seq_encoder, log_proj, wide_model, entry, step_to_path, bins, masks, device):
    """Text embedding (RoBERTa -> log_proj) concatenated with this instance's wide
    feature vector (ZOOMER's node/edge/IoC counts -> wide_model). One combined vector."""
...
def run_contrastive_train(n_epochs=N_EPOCHS, test_file_match=None, patience=PATIENCE, k_per_technique=K_PER_TACTIC,
    ...
    print('  Techniques (from full true multi-label membership of our {} steps): {}'.format(
        len(entries), ', '.join(all_techniques)))
    ...
    if test_file_match:
        ...
        print('  Split mode: stratified {:.0f}/{:.0f} (split_seed={}, every technique guaranteed >=1 test example)'.format(
    ...
    for t in all_techniques:
        print('    {} : {} train'.format(t, technique_counts[t]))
    ...
    if class_reweight:
        ...
        print('  Class reweighting ON -- per-technique loss weights (mean count / count, capped at {:.1f}x, '
              'rare techniques weighted up):'.format(reweight_cap))
        for t, w in zip(all_techniques, class_weights.tolist()):
    ...
    bins, masks, wide_dim = fit_wide_bins_and_masks(train_steps, step_to_path, seed=split_seed)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % 10 == 0 and best_state:
            ckpt_name = 'camlds_technique_wide_{}_epoch{}.pt'.format(run_tag, epoch)
    ...
    model_path = os.path.join(OUTPUT_TRAINING, 'camlds_technique_wide_matcher_{}.pt'.format(run_tag))
    torch.save({
    ...
    hist_path = os.path.join(RESULTS_DIR, 'train_history_camlds_technique_wide_{}.json'.format(run_tag))
    ...
    print()
    print('  Run test_camlds_matcher_technique_wide.py --run-tag {} for the detailed per-file test report.'.format(run_tag))


if __name__ == '__main__':
    import argparse
    ap = argparse.ArgumentParser()
    ap.add_argument('--k-per-technique', type=int, default=K_PER_TACTIC)
