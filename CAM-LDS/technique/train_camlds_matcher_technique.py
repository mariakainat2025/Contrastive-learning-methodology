...
def run_contrastive_train(n_epochs=N_EPOCHS, test_file_match=None, patience=PATIENCE, k_per_technique=K_PER_TACTIC,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % 10 == 0 and best_state:
            ckpt_name = 'camlds_technique_{}_epoch{}.pt'.format(run_tag, epoch)
    ...
    model_path = os.path.join(OUTPUT_TRAINING, 'camlds_technique_matcher_{}.pt'.format(run_tag))
    torch.save({
    ...
    hist_path = os.path.join(RESULTS_DIR, 'train_history_camlds_technique_{}.json'.format(run_tag))
    ...
    print()
    print('  Run test_camlds_matcher_technique.py --run-tag {} for the detailed per-file test report.'.format(run_tag))
