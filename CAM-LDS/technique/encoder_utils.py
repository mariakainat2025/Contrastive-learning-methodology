...
from config import MAX_LEN, STRIDE


def embed_text(model, tokenizer, input_ids, attention_mask, device,
               max_len=MAX_LEN, stride=STRIDE, truncate=False, max_chunks=None):
    ...
    if truncate:
        ...
        mask_exp = mask_t.unsqueeze(-1).float()
        return (out.last_hidden_state * mask_exp).sum(1) / mask_exp.sum(1).clamp(min=1)
    ...
    if short_texts:
        ...
        for idx, orig_idx in enumerate(short_indices):
            mask_exp = s_mask[idx].unsqueeze(-1).float()
            sentence_embeddings[orig_idx] = (out.last_hidden_state[idx] * mask_exp).sum(0) / mask_exp.sum().clamp(min=1)
    ...
    if long_texts:
        ...
        for ids_i, mask_i, orig_idx in long_texts:
            ...
            for start in range(1, seq_len, stride):
                if max_chunks is not None and n_chunks_this_seq >= max_chunks:
                    break
                n_chunks_this_seq += 1
        ...
        for cb_start in range(0, len(all_chunks), CHUNK_BATCH):
            ...
            for i, orig_idx in enumerate(chunk_meta[cb_start:cb_end]):
                mask_exp = c_mask[i].unsqueeze(-1).float()
                chunk_emb = (c_out.last_hidden_state[i] * mask_exp).sum(0) / mask_exp.sum().clamp(min=1)
                cls_per_orig.setdefault(orig_idx, []).append(chunk_emb)
