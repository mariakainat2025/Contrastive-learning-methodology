...
from train_camlds_matcher_technique import (
    load_sequences, leave_out_split, random_split, stratified_split, load_templates,
    ProjectionNetwork, SEED, MAX_CHUNKS,
)
...
def run(test_file_match=None, temp=0.07, test_size=0.2, split_seed=SEED, run_tag=None, min_events=None,
    ...
    ckpt_path = os.path.join(MODEL_DIR, 'camlds_technique_matcher_{}.pt'.format(run_tag))
    if not os.path.exists(ckpt_path):
        print('  ERROR: model not found at {}'.format(ckpt_path))
        print('  Run train_camlds_matcher_technique.py --run-tag {} first.'.format(run_tag))
        return
